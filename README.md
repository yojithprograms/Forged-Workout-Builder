...
`js/db.js` — defines `DB` and `EQ_ALLOWED` constants
`js/engine.js` — uses `DB`, `EQ_ALLOWED`, references `APP`
`js/render.js` — uses engine functions, references `APP`
`js/app.js` — defines `APP`, wires everything together, runs `init()`
---
How to Run
...
Filters `DB` by equipment tier (cumulative: `full gym` includes all)
Separates compounds and isolations; compounds sorted by fatigue score descending
Deprioritises recently used exercises (moves them to the end of the pool)
Uses a seeded deterministic pick — same inputs always produce the same plan
Targets ~45% compounds, remainder isolations
...
Your site will be live at `https://YOUR_USERNAME.github.io/forged`
---
Browser Support
